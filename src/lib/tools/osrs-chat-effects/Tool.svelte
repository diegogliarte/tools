...
{#if colour === "pattern"}
	<TextInput
		label="Pattern Key"
		placeholder="e.g. q3q3"
		bind:value={patternKey}
	/>
{/if}
...
{#key previewKey}
	<div class="mx-auto  overflow-hidden w-fit">
		{#if isWholeEffect}
...
<style>
    ...
    .flash1 { animation: flash1 0.8s infinite; }
    .flash2 { animation: flash2 0.8s infinite; }
    .flash3 { animation: flash3 0.8s infinite; }
    ...
    .glow1 { animation: glow1 2s linear infinite; }
    .glow2 { animation: glow2 2s linear infinite; }
    .glow3 { animation: glow3 2s linear infinite; }
